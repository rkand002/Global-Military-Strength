function Barcharts(clickWeapons)
{
  ...
  //   data.sort(function(a,b) { return b.value - a.value; });
    $('.OtherMaps h4.BrushibarHeading').text("List of Countries Possessing" +' '+ clickWeapons );
    $("#horizontalbar").empty();
    var bardic = {};
    var bararr1 = [];
      ...
      dataFromProjectJsonFile.forEach(function(item1){
        // if(item1[clickWeapons] != 0 )
        // {}
            var count = item1[clickWeapons].toString().replace(",","");
            
              bardic = {'key':count1,'country' : item1["Country"] , 'value' : parseInt(count)};
              bararr1.push(bardic);
              count1+=1;
              // console.log("kandi pappu "+bararr1); 
          
          // bardic[item1["Country"]] = item1[clickWeapons];
      });

      // var tempDic={}
      // bararr.forEach(function(d){
      //    if(!(tempDic.hasOwnProperty(d["label"])){
      //      tempDic[d["label"]]
      //    }
      // });

      bararr1.sort(function(a, b) {return parseInt(b.value) - parseInt(a.value); })
      bararr=bararr1;
      // bararr=bararr1.splice(0,10);
    /////////////////////////////////////////////////////////////
    ...
    var main_margin = {top: 10, right: 10, bottom: 60, left: 200},
        main_width = 500 - main_margin.left - main_margin.right,
        main_height = 400 - main_margin.top - main_margin.bottom;

    var mini_margin = {top: 10, right: 10, bottom: 60, left: 10},
        mini_height = 400 - mini_margin.top - mini_margin.bottom;
    mini_width = 100 - mini_margin.left - mini_margin.right;

    svg = d3.select("#horizontalbar").append("svg")
    ...
    mainGroup.append("g")
        ...
        svg.append("text").attr(
          "transform",
          "translate(" + (main_width / 2) + " ," + (main_height + main_margin.bottom)
              + ")").attr("y",7).attr("x", 200)
            .style("text-anchor", "middle").text("Count");
        
    
        svg.append("text").attr("transform", "rotate(-90)").attr("y",
            100).attr("x", -180).attr("dy", "1em")
            .style("text-anchor", "middle").text("Countries");
    ...
    //Update the scales
    main_xScale.domain([0, d3.max(bararr1, function(d) { return d.value; })]);
    mini_xScale.domain([0, d3.max(bararr1, function(d) { return d.value; })]);
    main_yScale.domain(bararr1.map(function(d) { return d.country; }));
    mini_yScale.domain(bararr1.map(function(d) { return d.country; }));
    ...
    var brushExtent = Math.max( 1, Math.min( 20, Math.round(bararr1.length*0.2) ) );

    brush = d3.svg.brush()
        .y(mini_yScale)
        .extent([mini_yScale(bararr1[0].country), mini_yScale(bararr1[brushExtent].country)])
        .on("brush", brushmove)
    ...
    var mini_bar = d3.select(".miniGroup").selectAll(".bar")
      .data(bararr1, function(d) { return d.key; });
  ...
  function update() {
    ...
    bar.enter().append("rect")
      ...
      .on("mouseover", function(d) { 
           ...
           console.log("d");
           $(".tooltipdiv").html("<h4>"+d.value+"</h4>");
            $(".tooltipdiv").css("display","block");
            $(".tooltipdiv").css("position","absolute");
            $(".tooltipdiv").css("top",d3.event.pageY-40+"px");
            $(".tooltipdiv").css("left",d3.event.pageX-10+"px");
